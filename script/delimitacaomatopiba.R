...
list_micro<-readr::read_csv("dados/micro.CSV",locale = readr::locale(encoding = "latin1"))
...
for(i in 1:length(codmicro)){
  temp<- geobr::read_micro_region(code_micro = codmicro[i], year=2020)
  micromtp<-rbind(micromtp,temp)
}

sf::st_write(micromtp, here::here("dados","shp","microrregioes_matopiba.shp"))
